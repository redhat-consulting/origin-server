...
module OpenShift
  class ActiveMQPlugin < OpenShift::RoutingService
    def initialize
      ...
      @dest = Rails.application.config.routing_xha_activemq[:destination]
      @hosts = Rails.application.config.routing_xha_activemq[:hosts]
      @mcollective_conf = Rails.application.config.routing_xha_activemq[:mcollective_conf]
      ...
      @hosts = @hosts.map do |host|
        host.merge({
          :login => Rails.application.config.routing_xha_activemq[:username],
          :passcode => Rails.application.config.routing_xha_activemq[:password],
        })
      end

      if Rails.application.config.routing_xha_activemq[:debug]
    ...
    def notify_add_alias(app, alias_str)
      ...
      # Get the head gear, and extract public IP and public port
      gear = app.get_app_dns_gear
      public_ip = gear.get_public_ip_address

      gear.port_interfaces.each do |pi|
        public_port = pi.external_port if pi.cartridge_name =~ /\Ahaproxy-.*\Z/
      end
